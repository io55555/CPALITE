...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	if cachedAuthID, ok := s.cache.GetAndRefresh(cacheKey); ok {
		for _, auth := range available {
			if auth.ID == cachedAuthID {
				entry.Debugf("session-affinity: cache hit | session=%s auth=%s api_key=%s provider=%s model=%s", truncateSessionID(primaryID), auth.ID, fullAPIKeyForLog(auth), provider, model)
				return auth, nil
			}
		}
		...
		entry.Debugf("session-affinity: cache hit but auth unavailable, reselected | session=%s auth=%s api_key=%s provider=%s model=%s", truncateSessionID(primaryID), auth.ID, fullAPIKeyForLog(auth), provider, model)
		return auth, nil
	}
	...
	if fallbackID != "" && fallbackID != primaryID {
		...
		if cachedAuthID, ok := s.cache.Get(fallbackKey); ok {
			for _, auth := range available {
				if auth.ID == cachedAuthID {
					s.cache.Set(cacheKey, auth.ID)
					entry.Debugf("session-affinity: fallback cache hit | session=%s fallback=%s auth=%s api_key=%s provider=%s model=%s", truncateSessionID(primaryID), truncateSessionID(fallbackID), auth.ID, fullAPIKeyForLog(auth), provider, model)
					return auth, nil
				}
			}
		}
	...
	s.cache.Set(cacheKey, auth.ID)
	entry.Debugf("session-affinity: cache miss, new binding | session=%s auth=%s api_key=%s provider=%s model=%s", truncateSessionID(primaryID), auth.ID, fullAPIKeyForLog(auth), provider, model)
	return auth, nil
}
...
func fullAPIKeyForLog(auth *Auth) string {
	if auth == nil {
		return ""
	}
