...
func (h *BaseAPIHandler) ExecuteModel(ctx context.Context, req ModelExecutionRequest) (ModelExecutionResponse, *interfaces.ErrorMessage) {
	...
	body, headers, errMsg := h.executeWithAuthManagerWithOptions(ctx, req.EntryProtocol, req.Model, cloneBytes(req.Body), req.Alt, false, modelExecutionOptions{
		Headers:                 req.Headers,
		Query:                   req.Query,
		ResponseFormat:          modelExecutionResponseProtocol(req.EntryProtocol, req.ExitProtocol),
		InternalSource:          true,
...
func (h *BaseAPIHandler) ExecuteModelStream(ctx context.Context, req ModelExecutionRequest) (ModelExecutionStream, *interfaces.ErrorMessage) {
	...
	dataChan, headers, errChan := h.executeStreamWithAuthManagerWithOptions(ctx, req.EntryProtocol, req.Model, cloneBytes(req.Body), req.Alt, false, modelExecutionOptions{
		Headers:                 req.Headers,
		Query:                   req.Query,
		ResponseFormat:          modelExecutionResponseProtocol(req.EntryProtocol, req.ExitProtocol),
		InternalSource:          true,
