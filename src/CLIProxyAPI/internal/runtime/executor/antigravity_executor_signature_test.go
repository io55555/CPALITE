...
func TestAntigravityExecutor_StrictBypassStripsInvalidSignature(t *testing.T) {
	...
	output, err := validateAntigravityRequestSignatures(context.Background(), "claude-sonnet-4-20250514", from, payload)
	if err != nil {
...
func TestAntigravityExecutor_StrictBypassLogsStrippedInvalidSignature(t *testing.T) {
	...
	if _, err := validateAntigravityRequestSignatures(context.Background(), "claude-sonnet-4-20250514", from, payload); err != nil {
		t.Fatalf("strict bypass should strip invalid signatures instead of rejecting request: %v", err)
	}
...
func TestAntigravityExecutor_NonStrictBypassSkipsPrecheck(t *testing.T) {
	...
	_, err := validateAntigravityRequestSignatures(context.Background(), "claude-sonnet-4-20250514", from, payload)
	if err != nil {
