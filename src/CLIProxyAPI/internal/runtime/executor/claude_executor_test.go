...
func TestClaudeExecutor_ReusesUserIDAcrossModelsWhenCacheEnabled(t *testing.T) {
	...
	t.Logf("鉁?End-to-end test passed: Same user_id (%s) was used for both models", userIDs[0])
}
...
func TestApplyCloaking_PreservesConfiguredStrictModeAndSensitiveWordsWhenModeOmitted(t *testing.T) {
	...
	out, errApply := applyCloaking(context.Background(), cfg, auth, payload, "claude-3-5-sonnet-20241022", "key-123")
	if errApply != nil {
		t.Fatalf("applyCloaking() error = %v", errApply)
	}
...
func TestRemapOAuthToolNames_MixedCase_OnlyRenamedToolsReversed(t *testing.T) {
	...
	// Upstream responds with a `Glob` tool_use. Since we renamed `glob`鈫抈Glob`,
