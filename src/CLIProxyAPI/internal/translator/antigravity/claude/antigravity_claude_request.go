...
func ConvertClaudeRequestToAntigravity(modelName string, inputRawJSON []byte, _ bool) []byte {
	...
	// tool_use_id 鈫?tool_name lookup, populated incrementally during the main loop.
					...
					if contentTypeResult.Type == gjson.String && contentTypeResult.String() == "thinking" {
						...
						if toolCallID != "" {
							...
							if !ok {
								...
								// the last two dash-separated segments (e.g. "get_weather-call-123" 鈫?"get_weather").
							...
							if functionResponseResult.Type == gjson.String {
								...
								// Content field is missing entirely 鈥?.Raw is empty which
