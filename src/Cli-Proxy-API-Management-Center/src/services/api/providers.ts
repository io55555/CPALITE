...
export const providersApi = {
  saveGeminiKeys: (configs: GeminiKeyConfig[]) =>
    apiClient.put(
      '/gemini-api-key',
      configs.map((config) =>
        mergeProviderKeyPayload(undefined, serializeGeminiKey(config), GEMINI_KEY_FIELDS)
      )
    ),
  ...
  saveCodexConfigs: (configs: ProviderKeyConfig[]) =>
    apiClient.put(
      '/codex-api-key',
      configs.map((config) =>
        mergeProviderKeyPayload(undefined, serializeProviderKey(config), CODEX_KEY_FIELDS)
      )
    ),
  ...
  saveClaudeConfigs: (configs: ProviderKeyConfig[]) =>
    apiClient.put(
      '/claude-api-key',
      configs.map((config) =>
        mergeProviderKeyPayload(undefined, serializeProviderKey(config), CLAUDE_KEY_FIELDS)
      )
    ),
  ...
  saveVertexConfigs: (configs: ProviderKeyConfig[]) =>
    apiClient.put(
      '/vertex-api-key',
      configs.map((config) =>
        mergeProviderKeyPayload(undefined, serializeVertexKey(config), VERTEX_KEY_FIELDS)
      )
    ),
  ...
  saveOpenAIProviders: (providers: OpenAIProviderConfig[]) =>
    apiClient.put(
      ...
      providers.map((provider) =>
        mergeOpenAIProviderPayload(undefined, serializeOpenAIProvider(provider))
      )
    ),
  ...
  deleteOpenAIProvider: (indexOrName: number | string) =>
    typeof indexOrName === 'number'
      ? apiClient.delete(`/openai-compatibility?index=${encodeURIComponent(String(indexOrName))}`)
      : apiClient.delete(`/openai-compatibility?name=${encodeURIComponent(indexOrName)}`),
  ...
  getOpenAIKeyStateDetail: (providerName: string, apiKey: string): Promise<OpenAIKeyState> =>
    apiClient.get(
      `/openai-compatibility/key-state/detail?provider_name=${encodeURIComponent(providerName)}&api_key=${encodeURIComponent(apiKey)}`
    ),
