...
export function LogsPage() {
  ...
  // 保存最新时间戳用于增量获取
  const latestTimestampRef = useRef<number | string | undefined>(undefined);

  const disableControls = connectionStatus !== 'connected';
  const refreshDisabled = disableControls || loading || showFileLoggingRequired;
  const autoRefreshDisabled = disableControls || showFileLoggingRequired;
  const clearDisabled = disableControls || loading || showFileLoggingRequired;
  ...
  async function loadLogs(incremental = false) {
    ...
    try {
      ...
      const params =
        incremental && latestTimestampRef.current !== undefined
          ? { after: latestTimestampRef.current }
          : {};
      const data = await logsApi.fetchLogs(params);

      // 更新时间戳
      if (data.latestCursor !== undefined) {
        latestTimestampRef.current = data.latestCursor;
      }
