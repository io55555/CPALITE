...
const actionOptions = [
  ...
  { value: 'modify_status', label: '修改响应码' },
  ...
  { value: 'return_clean_404_model_not_support', label: '返回客户端纯净404(模型不支持)' },
...
const replacementOptions = [
  ...
  { value: '404', label: '响应码 404' },
  { value: '{"error":"model not support"}', label: '模型不支持JSON' },
  { value: '1', label: '数值 1' },
...
const ruleTemplates: RuleTemplate[] = [
  ...
  {
    value: 'client-response-ollama-subscription-404',
    label: 'ollama过滤模型需要订阅',
    rule: {
      name: '[CPA到客户端]ollama过滤模型需要订阅',
      provider: 'ollama_cloud',
      packet: 'client_response',
      part: 'status',
      operator: 'num_eq',
      value_number: 403,
      action: 'return_clean_404_model_not_support',
      target: 'response',
      match_logic: 'all',
      conditions: [
        { packet: 'client_response', part: 'status', operator: 'num_eq', value_number: 403 },
        { packet: 'client_response', part: 'body_json', json_path: 'error', operator: 'contains', value: 'model requires a subscription' },
      ],
      actions: [
        { type: 'return_clean_404_model_not_support', packet: 'client_response', target: 'response' },
      ],
      notes: '命中 ollama_cloud 返回的订阅模型错误后，客户端只收到 404 和 {"error":"model not support"}。',
    },
  },
  {
