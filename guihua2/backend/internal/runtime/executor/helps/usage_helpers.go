...
func (r *UsageReporter) CaptureThinkingEffort(body []byte, model, fromFormat, toFormat string) {
	...
	r.thinkingOnce.Do(func() {
		r.thinkingEffort = extractThinkingEffort(body, model, fromFormat, toFormat)
	})
}
...
func extractThinkingEffort(body []byte, model, fromFormat, toFormat string) string {
	_ = fromFormat
	_ = toFormat
	if len(body) == 0 || !gjson.ValidBytes(body) {
		return strings.TrimSpace(parseThinkingSuffix(model))
	}
	for _, candidate := range []string{
		...
		gjson.GetBytes(body, "generationConfig.thinkingConfig.thinkingLevel").String(),
		gjson.GetBytes(body, "generationConfig.thinkingConfig.thinking_level").String(),
	} {
		if trimmed := strings.TrimSpace(candidate); trimmed != "" {
			return trimmed
		}
	}
	if trimmed := strings.TrimSpace(parseThinkingSuffix(model)); trimmed != "" {
		return trimmed
	}
	return ""
}

func parseThinkingSuffix(model string) string {
	model = strings.TrimSpace(model)
	if model == "" {
		return ""
	}
	...
	return strings.TrimSpace(model[open+1 : close])
}
