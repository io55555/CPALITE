...
const storeInsertTimeout = 5 * time.Second
...
func (p *LoggerPlugin) HandleUsage(ctx context.Context, record coreusage.Record) {
	...
	if store := DefaultStore(); store != nil {
		insertCtx, cancel := context.WithTimeout(context.Background(), storeInsertTimeout)
		defer cancel()
		if err := store.Insert(insertCtx, persistRecord(ctx, record)); err != nil {
			log.Warnf("usage sqlite insert failed: %v", err)
		}
	}
}
...
func (s *RequestStatistics) Record(ctx context.Context, record coreusage.Record) {
	...
	s.updateAPIStats(stats, modelName, RequestDetail{
		...
		GenerationMs: normaliseGenerationLatency(record.Latency, record.FirstByteLatency),
...
func (s *RequestStatistics) updateAPIStats(stats *apiStats, model string, detail RequestDetail) {
	...
	if !shouldRetainInMemoryDetails() {
		return
	}
	...
	for apiName, apiSnapshot := range snapshot.APIs {
		...
		for modelName, modelSnapshot := range apiSnapshot.Models {
			...
			for _, detail := range modelSnapshot.Details {
				...
				if detail.GenerationMs < 0 {
					detail.GenerationMs = 0
				}
...
func dedupKey(apiName, modelName string, detail RequestDetail) string {
	...
	return fmt.Sprintf(
		"%s|%s|%s|%s|%s|%s|%t|%d|%d|%d|%s|%d|%d|%d|%d|%d",
		apiName,
		modelName,
		timestamp,
		detail.Source,
		...
		detail.GenerationMs,
		strings.TrimSpace(detail.ThinkingEffort),
		tokens.InputTokens,
		tokens.OutputTokens,
...
func normaliseGenerationLatency(latency, firstByteLatency time.Duration) int64 {
	total := normaliseLatency(latency)
	firstByte := normaliseLatency(firstByteLatency)
	if total <= firstByte {
		return 0
	}
...
func persistRecord(ctx context.Context, record coreusage.Record) PersistedRecord {
	timestamp := record.RequestedAt
	if timestamp.IsZero() {
		timestamp = time.Now()
	}
...
func shouldRetainInMemoryDetails() bool {
