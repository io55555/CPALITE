...
func (s *Service) applyCoreAuthAddOrUpdate(ctx context.Context, auth *coreauth.Auth) {
	...
	if existing, ok := s.coreManager.GetByID(auth.ID); ok {
		...
		if shouldPreserveOpenAICompatRuntimeState(existing, auth) {
			...
			if len(existing.Metadata) > 0 {
				auth.Metadata = existing.Metadata
			}
		}
...
func shouldPreserveOpenAICompatRuntimeState(existing, next *coreauth.Auth) bool {
	if existing == nil || next == nil {
		return false
	}
	_, _, existingCompat := openAICompatInfoFromAuth(existing)
	_, _, nextCompat := openAICompatInfoFromAuth(next)
...
func (s *Service) Run(ctx context.Context) error {
	...
	defer func() {
		if err := capture.CloseDefaultStore(); err != nil {
			log.Warnf("failed to close capture store: %v", err)
		}
		if err := statusruler.CloseDefaultStore(); err != nil {
			log.Warnf("failed to close status-ruler store: %v", err)
		}
...
func (s *Service) Shutdown(ctx context.Context) error {
	...
	s.shutdownOnce.Do(func() {
		...
		usage.StopDefault()
		if err := internalusage.CloseDefaultStore(); err != nil {
			log.Warnf("failed to close usage sqlite store: %v", err)
		}
	})
	return shutdownErr
}
