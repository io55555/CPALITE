...
func main() {
	...
	if err := capture.InitDefaultStore(filepath.Join(logDir, "capture.db")); err != nil {
		log.Warnf("failed to initialize capture sqlite store: %v", err)
	}
	if err := statusruler.InitDefaultStore(filepath.Join(logDir, "status-ruler.db")); err != nil {
		log.Warnf("failed to initialize status-ruler sqlite store: %v", err)
	}
