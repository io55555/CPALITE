...
func (h *Handler) PatchOpenAICompat(c *gin.Context) {
	type openAICompatPatch struct {
		...
		StatusRules   *[]config.OpenAICompatStatusRule    `json:"status-rules"`
		Headers       *map[string]string                  `json:"headers"`
	}
	var body struct {
		Name  *string            `json:"name"`
	...
	if body.Value.StatusRules != nil {
		entry.StatusRules = append([]config.OpenAICompatStatusRule(nil), (*body.Value.StatusRules)...)
	}
	if body.Value.Headers != nil {
		entry.Headers = config.NormalizeHeaders(*body.Value.Headers)
	}
...
func normalizeOpenAICompatibilityEntry(entry *config.OpenAICompatibility) {
	...
	if len(entry.StatusRules) > 0 {
		entry.StatusRules = append([]config.OpenAICompatStatusRule(nil), entry.StatusRules...)
	}
