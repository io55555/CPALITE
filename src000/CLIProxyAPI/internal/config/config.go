...
type OpenAICompatibility struct {
	...
	// StatusRules defines custom status/body matching rules for auto disable/freeze actions.
	StatusRules []OpenAICompatStatusRule `yaml:"status-rules,omitempty" json:"status-rules,omitempty"`
...
// OpenAICompatStatusRule defines a lightweight response matching rule for
// OpenAI-compatible upstreams. Matched rules can disable a key or freeze it for
// a duration, independent from the static config file.
type OpenAICompatStatusRule struct {
	Name string `yaml:"name,omitempty" json:"name,omitempty"`

	// Status limits the rule to one HTTP status code. 0 means any status.
	Status int `yaml:"status,omitempty" json:"status,omitempty"`

	// BodyEquals requires the full response body to equal this string after trim.
	BodyEquals string `yaml:"body-equals,omitempty" json:"body-equals,omitempty"`
	// BodyContains requires the response body to contain this string.
	BodyContains string `yaml:"body-contains,omitempty" json:"body-contains,omitempty"`

	// JSONPath reads a field from a JSON body using dot syntax, e.g. "error.code".
	JSONPath string `yaml:"json-path,omitempty" json:"json-path,omitempty"`
	// JSONEquals requires the extracted JSON field to equal this string.
	JSONEquals string `yaml:"json-equals,omitempty" json:"json-equals,omitempty"`
	// JSONContains requires the extracted JSON field to contain this string.
	JSONContains string `yaml:"json-contains,omitempty" json:"json-contains,omitempty"`

	// JSONCode is a short-hand for JSONPath=error.code + JSONEquals=<value>.
	JSONCode string `yaml:"json-code,omitempty" json:"json-code,omitempty"`

	// Action supports disable / freeze / freeze-30m / freeze-12h / freeze-7d.
...
func (cfg *Config) SanitizeOpenAICompatibility() {
	...
	for i := range cfg.OpenAICompatibility {
		...
		for j := range e.StatusRules {
			e.StatusRules[j].Name = strings.TrimSpace(e.StatusRules[j].Name)
			e.StatusRules[j].BodyEquals = strings.TrimSpace(e.StatusRules[j].BodyEquals)
			e.StatusRules[j].BodyContains = strings.TrimSpace(e.StatusRules[j].BodyContains)
			e.StatusRules[j].JSONPath = strings.TrimSpace(e.StatusRules[j].JSONPath)
			e.StatusRules[j].JSONEquals = strings.TrimSpace(e.StatusRules[j].JSONEquals)
			e.StatusRules[j].JSONContains = strings.TrimSpace(e.StatusRules[j].JSONContains)
			e.StatusRules[j].JSONCode = strings.TrimSpace(e.StatusRules[j].JSONCode)
			e.StatusRules[j].Action = strings.TrimSpace(e.StatusRules[j].Action)
		}
		if e.BaseURL == "" {
