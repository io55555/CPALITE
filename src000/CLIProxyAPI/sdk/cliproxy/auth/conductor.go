				...
				if !isRequestScopedNotFoundResultError(result.Error) {
					...
					if action, matched := matchOpenAICompatStatusRule(m.runtimeConfigValue(), auth, statusCode, errorBodyFromResult(result.Error)); matched {
						...
						if action.disable {
							...
							auth.StatusMessage = action.label
							state.Status = StatusDisabled
							state.Unavailable = true
							state.NextRetryAfter = time.Time{}
							state.StatusMessage = action.label
						} else if action.freeze > 0 {
							next := now.Add(action.freeze)
							auth.Disabled = false
...
func (m *Manager) runtimeConfigValue() *internalconfig.Config {
	if m == nil {
...
func errorBodyFromResult(err *Error) string {
	if err == nil {
		return ""
	}
