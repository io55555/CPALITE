...
const maskApiKeyBrief = (value: string) => {
  const trimmed = value.trim();
  if (!trimmed) return '';
...
const buildEventRows = (
  ...
  details
    .map((detail, index) => {
      ...
      const apiKeyMasked = maskApiKeyBrief(readString(detail.api_key));
      const account = apiKeyMasked || authMeta?.account || sourceLabel;
      const accountMasked = apiKeyMasked || maskEmailLike(account);
