...
export function AiProvidersOpenAIEditPage() {
  ...
  const [keyErrorDetails, setKeyErrorDetails] = useState<Record<number, string>>({});
  const [detailIndex, setDetailIndex] = useState<number | null>(null);
  const [runtimeBusyIndex, setRuntimeBusyIndex] = useState<number | null>(null);
  ...
  const buildKeyErrorDetail = useCallback(
    (
      keyIndex: number,
      endpoint: string,
      modelName: string,
      ...
      return [
        ...
        `ProxyURL: ${keyEntry?.proxyUrl?.trim() || '(跟随已保存配置/全局代理/直连)'}`,
        '',
        '[请求头]',
        ...
        requestBody,
        '',
        transportError ? '[请求错误]' : '[响应状态]',
        transportError || String(responseStatus ?? 0),
        '',
        '[响应头]',
  ...
  const updateKeyRuntimeDisabled = useCallback(
    async (keyIndex: number, disabled: boolean) => {
      const keyEntry = form.apiKeyEntries[keyIndex];
      if (!keyEntry?.authIndex) {
        showNotification('请先保存提供商后再切换密钥启停状态', 'warning');
        return;
      }
      setRuntimeBusyIndex(keyIndex);
      try {
        await providersApi.updateOpenAICompatRuntimeState(keyEntry.authIndex, { disabled });
        setForm((prev) => ({
          ...prev,
        ...
        showNotification(`切换密钥状态失败: ${getErrorMessage(err)}`, 'error');
      } finally {
        setRuntimeBusyIndex(null);
      }
    },
    [form.apiKeyEntries, setForm, showNotification]
  );
  ...
  const runSingleKeyTest = useCallback(
    async (keyIndex: number): Promise<boolean> => {
      ...
      try {
        ...
        if (result.statusCode < 200 || result.statusCode >= 300) {
          ...
          setKeyErrorDetails((prev) => ({
            ...prev,
            [keyIndex]: buildKeyErrorDetail(
              keyIndex,
              endpoint,
              modelName,
              headers,
    ...
    [availableModels, buildKeyErrorDetail, form.apiKeyEntries, form.baseUrl, form.headers, setDraftKeyTestStatus, showNotification, t, testModel]
  );
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        title={detailIndex !== null ? `Key #${detailIndex + 1} 错误详情` : '错误详情'}
        width={920}
