...
import { apiCallApi, getApiCallErrorMessage, logsApi } from '@/services/api';
...
const buildFailureDetailText = (requestId: string, body: string) =>
  [`request_id: ${requestId}`, '', body.trim()].filter(Boolean).join('\n');
...
export function MonitoringCenterPage() {
  ...
  const [failureDetailTitle, setFailureDetailTitle] = useState('');
  const [failureDetailText, setFailureDetailText] = useState('');
  ...
  const openFailureDetail = useCallback(
    async (row: MonitoringEventRow) => {
      if (!row.requestId) {
        setFailureDetailTitle('失败详情');
        setFailureDetailText('当前记录没有 request_id，无法读取完整请求/响应日志。');
        setFailureDetailOpen(true);
        return;
      }
      setFailureDetailTitle(`失败详情 · ${row.model}`);
      setFailureDetailText('');
      setFailureDetailOpen(true);
      setFailureDetailLoading(true);
      try {
        const text = await logsApi.fetchRequestLogByIdText(row.requestId);
        setFailureDetailText(buildFailureDetailText(row.requestId, text));
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err || '');
        setFailureDetailText(
          buildFailureDetailText(
            row.requestId,
            message ? `读取失败: ${message}` : '读取失败，未拿到请求日志。'
          )
        );
      } finally {
  ...
  return (
    <div className={styles.page}>
                  ...
                  <td>
                    {row.failed ? (
                      <button
                        type="button"
                        className={styles.inlineActionButton}
                        onClick={() => void openFailureDetail(row)}
                        title={row.requestId ? '点击查看完整失败详情' : '当前记录缺少 request_id'}
                      >
      ...
      <Modal
        ...
        title={failureDetailTitle || '失败详情'}
        width={920}
        className={styles.monitorModal}
      >
        {failureDetailLoading ? (
          <div className={styles.emptyBlockSmall}>{t('common.loading')}</div>
        ) : (
          <pre className={styles.rawLog} style={{ maxHeight: '65vh', overflow: 'auto' }}>
            {failureDetailText || '没有可显示的失败详情。'}
          </pre>
        )}
      </Modal>
