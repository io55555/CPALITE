...
const normalizeApiKeyEntry = (entry: unknown): ApiKeyEntry | null => {
  ...
  const statusMessage = normalizeString(record?.['status-message'] ?? record?.statusMessage);
  if (statusMessage) result.statusMessage = statusMessage;
  const lastError = normalizeString(record?.['last-error'] ?? record?.lastError);
  if (lastError) result.lastError = lastError;
  return result;
};

const normalizeOpenAIStatusRules = (input: unknown): OpenAIStatusRule[] => {
  if (!Array.isArray(input)) return [];
  return input
    .map((item) => {
      if (!isRecord(item)) return null;
      const rule: OpenAIStatusRule = {};
      const name = normalizeString(item.name);
      const status = normalizeNumber(item.status);
      const bodyEquals = normalizeString(item['body-equals'] ?? item.bodyEquals);
      const bodyContains = normalizeString(item['body-contains'] ?? item.bodyContains);
      const jsonPath = normalizeString(item['json-path'] ?? item.jsonPath);
      const jsonEquals = normalizeString(item['json-equals'] ?? item.jsonEquals);
      const jsonContains = normalizeString(item['json-contains'] ?? item.jsonContains);
      const jsonCode = normalizeString(item['json-code'] ?? item.jsonCode);
      const action = normalizeString(item.action);
      if (name) rule.name = name;
      if (status !== undefined) rule.status = status;
      if (bodyEquals) rule.bodyEquals = bodyEquals;
      if (bodyContains) rule.bodyContains = bodyContains;
      if (jsonPath) rule.jsonPath = jsonPath;
      if (jsonEquals) rule.jsonEquals = jsonEquals;
      if (jsonContains) rule.jsonContains = jsonContains;
      if (jsonCode) rule.jsonCode = jsonCode;
      if (action) rule.action = action;
      return Object.keys(rule).length > 0 ? rule : null;
    })
    .filter(Boolean) as OpenAIStatusRule[];
};
...
const normalizeOpenAIProvider = (provider: unknown): OpenAIProviderConfig | null => {
  ...
  if (authIndex) result.authIndex = authIndex;
  const statusRules = normalizeOpenAIStatusRules(
    provider['status-rules'] ?? provider.statusRules ?? provider['status_rules']
  );
  if (statusRules.length) result.statusRules = statusRules;
  return result;
};
