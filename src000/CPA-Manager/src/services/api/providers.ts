...
const serializeOpenAIStatusRules = (rules?: OpenAIStatusRule[]) =>
  Array.isArray(rules)
    ? rules
        .map((rule) => {
          const payload: Record<string, unknown> = {};
          if (rule.name?.trim()) payload.name = rule.name.trim();
          if (rule.status !== undefined) payload.status = rule.status;
          if (rule.bodyEquals?.trim()) payload['body-equals'] = rule.bodyEquals.trim();
          if (rule.bodyContains?.trim()) payload['body-contains'] = rule.bodyContains.trim();
          if (rule.jsonPath?.trim()) payload['json-path'] = rule.jsonPath.trim();
          if (rule.jsonEquals?.trim()) payload['json-equals'] = rule.jsonEquals.trim();
          if (rule.jsonContains?.trim()) payload['json-contains'] = rule.jsonContains.trim();
          if (rule.jsonCode?.trim()) payload['json-code'] = rule.jsonCode.trim();
          if (rule.action?.trim()) payload.action = rule.action.trim();
...
const serializeOpenAIProvider = (provider: OpenAIProviderConfig) => {
  ...
  const statusRules = serializeOpenAIStatusRules(provider.statusRules);
  if (statusRules && statusRules.length) payload['status-rules'] = statusRules;
  return payload;
};
...
export const providersApi = {
  ...
  updateOpenAICompatRuntimeState: (
    authIndex: string,
    payload: { disabled?: boolean; action?: 'enable' | 'disable' | 'freeze' }
  ) => apiClient.patch('/openai-compatibility/runtime-state', { 'auth-index': authIndex, ...payload }),
